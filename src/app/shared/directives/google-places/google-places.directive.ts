...
export class GooglePlacesDirective implements OnInit {
  ...
  getFormattedAddress(place: any, format?: any) {
    //@params: place - Google Autocomplete place object
    //@returns: location_obj - An address object in human readable format
    let location_obj: any = {};
    for (let i in place.address_components) {
      let item = place.address_components[i];
      if (!!format) location_obj['formatted_address'] = format;
      else location_obj['formatted_address'] = place.formatted_address;
  ...
  ngOnInit() {
    ...
    google.maps.event.addListener(autocomplete, 'place_changed', () => {
      ...
      this.onSelect.emit(this.getFormattedAddress(places, this.element.value));
    });
  }
}
